...
import { initLoader, selectAllModule, selectModule, selectReadOnly, updatePromoterRole } from "./promoter_role_slice";
import { Loading } from "../../components/Loading";

export const AdmPromoterRole = ({ open, setOpen, data }) => {
    ...
    return <> 
        ...
        <Modal md={6} sm={6} xs={10} open={open} setOpen={setOpen} onClose={() => { setOpen(false); }} title={("user role setup")} outsideclick>
            <CardHeaderButton>
                ...
                <PrimaryButton
                    onClick={() => { dispatch(updatePromoterRole(data)) }}
                >
                    { ("Submit")}
                </PrimaryButton>
