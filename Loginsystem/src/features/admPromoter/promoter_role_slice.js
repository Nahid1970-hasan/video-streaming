...
export const loadPromoterRole = createAsyncThunk("AdmPromoterRole/loadRole", (data) => {
  return socket.post({ type: "get_promoters_roles", data });
});

export const updatePromoterRole = createAsyncThunk(
...
const UserRoleSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder.addCase(loadPromoterRole.pending, (state, action) => {
      state.loading = "pending";
    });
    builder.addCase(loadPromoterRole.fulfilled, (state, action) => {
      state.read_only = action.payload.data.read_only;
      state.list = action.payload.data.user_modules;
      state.loading = "succeeded";
    });
    builder.addCase(loadPromoterRole.rejected, (state, action) => {
      state.msg = action.error.message;
      state.loading = action.error.name;
    });
    builder.addCase(updatePromoterRole.pending, (state) => {
      state.updateLoading = "pending";
    });
    builder.addCase(updatePromoterRole.fulfilled, (state, action) => {
      state.updateLoading = "succeeded";
      state.msg = action.payload.data.msg;
    });
    builder.addCase(updatePromoterRole.rejected, (state, action) => {
      state.msg = action.error.message;
