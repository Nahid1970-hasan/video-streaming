...
import { initLoader, selectAllModule, selectModule, selectReadOnly, updateSubscriberRole} from "./subscriber_role_slice";
import { Loading } from "../../components/Loading";

export const AdmSubscriberRole = ({ open, setOpen, data }) => {
    ...
    return <> 
        ...
        <Modal md={6} sm={6} xs={10} open={open} setOpen={setOpen} onClose={() => { setOpen(false); }} title={("user role setup")} outsideclick>
            <CardHeaderButton>
                <div>
                    <label htmlFor="readonly">{('read_only_user')}</label>
                ...
                <PrimaryButton
                    onClick={() => { dispatch(updateSubscriberRole(data)) }}
                >
                    { ("Submit")}
                </PrimaryButton>
