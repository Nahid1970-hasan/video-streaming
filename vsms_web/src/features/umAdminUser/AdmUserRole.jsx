...
import { initLoader, selectAllModule, selectModule, selectReadOnly, updateUserRole } from "./um_admin_user_role_slice";
import { Loading } from "../../components/Loading";
import { Toast } from "../../components/Toast";

export const UMAdmUserRole = ({ open, setOpen, data }) => {

    const UserRole = useSelector(state => state.umadmuserrole);
     ...
     {(UserRole.updateLoading == "idle" || UserRole.updateLoading == "pending") ? <></> : (
        ...
        <Modal md={6} sm={6} xs={10} open={open} setOpen={setOpen} onClose={() => { setOpen(false); }} title={("User Role Setup")} outsideclick>
            <CardHeaderButton>
                ...
                <PrimaryButton 
                    onClick={() => { console.log(data), dispatch(updateUserRole(data)) }}  >
                    { ("Submit")}
                </PrimaryButton>
